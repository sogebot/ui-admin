...
import type { ClientToServerEventsWithNamespace, Fn } from '../backend/d.ts/src/helpers/socket';
...
export function getSocket<K0 extends keyof O, O extends Record<PropertyKey, Record<PropertyKey, Fn>> = ClientToServerEventsWithNamespace> (namespace: K0, continueOnUnauthorized = false): Socket<O[K0]> {
  ...
  const wsUrl = JSON.parse(location.origin).replace('https', '').replace('http', '');
  sessionStorage.wsUrl = `${(JSON.parse(location.origin).startsWith('https') ? 'wss' : 'ws')}${wsUrl}`;
