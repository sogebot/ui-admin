<template>
  ...
  <v-form v-else v-model="valid" lazy-validation>
    <v-tabs v-model="tab">
      <v-tab>{{translate('categories.conversion')}}</v-tab>
      <v-tab>{{translate('categories.levels')}}</v-tab>
      <v-tab>{{translate('categories.xp')}}</v-tab>
    </v-tabs>
    ...
    <v-tabs-items v-model="tab">
      <v-tab-item eager>
        <v-card>
          <v-card-text>
            <revert-text-field v-model="settings.conversion.conversionRate" type="number" min="1"
              :label="translate('systems.levels.settings.conversionRate')" :rules="[required, minValue(1)]" />
          </v-card-text>
        </v-card>
      </v-tab-item>
      <v-tab-item eager>
        <v-card>
          <v-card-text>
            <revert-text-field v-model="settings.levels.firstLevelStartsAt" type="number" min="1"
              :label="translate('systems.levels.settings.firstLevelStartsAt')" :rules="[required, minValue(1)]" />
            <revert-text-field v-model="settings.levels.nextLevelFormula"
              :label="translate('systems.levels.settings.nextLevelFormula.title')"
              :hint="translate('systems.levels.settings.nextLevelFormula.help')" :rules="[required]" />
            ...
            <v-row no-gutters>
              <v-col>
                <v-simple-table dense>
                  <template v-slot:default>
                    <tbody>
                      <tr v-for="(xp, idx) in showcase" v-show="idx > 0 && idx < 8" :key="'Level '+ idx + ': ' + xp">
                        <td>
                          {{ idx }}
                        </td>
                        <td>
              ...
              <v-col>
                <v-simple-table dense>
                  <template v-slot:default>
                    <tbody>
                      <tr v-for="(xp, idx) in showcase" v-show="idx >= 8 && idx < 15" :key="'Level '+ idx + ': ' + xp">
                        <td>
                          {{ idx }}
                        </td>
                        <td>
              ...
              <v-col>
                <v-simple-table dense>
                  <template v-slot:default>
                    <tbody>
                      <tr v-for="(xp, idx) in showcase" v-show="idx >= 15" :key="'Level '+ idx + ': ' + xp">
                        <td>
                          {{ idx }}
                        </td>
                        <td>
      ...
      <v-tab-item eager>
        <v-card>
          <v-card-text>
            <revert-text-field v-model="settings.xp.xpName" :label="translate('systems.levels.settings.xpName')"
              :rules="[required]" />
