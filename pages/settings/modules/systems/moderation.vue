  ...
  <v-card v-else flat style="min-height: 100%;">
    <v-card-text>
      <v-form ref="form" v-model="valid">
        ...
        <v-card-title>{{ translate('categories.lists') }}</v-card-title>
        <v-textarea
          auto-grow
          outlined
          type="password"
          persistent-hint
          @focus="maskBlacklist = false;"
          @blur="maskBlacklist = true;"
          :label="translate('systems.moderation.settings.cListsBlacklist')"
          ...
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.lists.cListsEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.lists.cListsEnabled[0])">
                  <v-icon>{{ settings.__permission_based__.lists.cListsEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.lists.cListsTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.lists.cListsTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.lists.cListsTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.caps_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'caps_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.caps_filter.cCapsEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.caps_filter.cCapsEnabled[0])">
                  <v-icon>{{ settings.__permission_based__.caps_filter.cCapsEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.caps_filter.cCapsMaxCapsPercent[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.caps_filter.cCapsMaxCapsPercent[0])">
                  <v-icon>{{ settings.__permission_based__.caps_filter.cCapsMaxCapsPercent[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.caps_filter.cCapsTriggerLength[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.caps_filter.cCapsTriggerLength[0])">
                  <v-icon>{{ settings.__permission_based__.caps_filter.cCapsTriggerLength[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.caps_filter.cCapsTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.caps_filter.cCapsTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.caps_filter.cCapsTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.color_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'color_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.color_filter.cColorEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.color_filter.cColorEnabled[0])">
                  <v-icon>{{ settings.__permission_based__.color_filter.cColorEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.color_filter.cColorTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.color_filter.cColorTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.color_filter.cColorTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.emotes_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'emotes_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.emotes_filter.cEmotesEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.emotes_filter.cEmotesEnabled[0])">
                    <v-icon>{{ settings.__permission_based__.emotes_filter.cEmotesEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.emotes_filter.cEmotesEmojisAreEmotes[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.emotes_filter.cEmotesEmojisAreEmotes[0])">
                    <v-icon>{{ settings.__permission_based__.emotes_filter.cEmotesEmojisAreEmotes[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.emotes_filter.cEmotesMaxCount[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.emotes_filter.cEmotesMaxCount[0])">
                  <v-icon>{{ settings.__permission_based__.emotes_filter.cEmotesMaxCount[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.emotes_filter.cEmotesTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.emotes_filter.cEmotesTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.emotes_filter.cEmotesTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.links_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'links_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.links_filter.cLinksEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.links_filter.cLinksEnabled[0])">
                    <v-icon>{{ settings.__permission_based__.links_filter.cLinksEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.links_filter.cLinksIncludeClips[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.links_filter.cLinksIncludeClips[0])">
                    <v-icon>{{ settings.__permission_based__.links_filter.cLinksIncludeClips[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.links_filter.cLinksIncludeSpaces[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.links_filter.cLinksIncludeSpaces[0])">
                    <v-icon>{{ settings.__permission_based__.links_filter.cLinksIncludeSpaces[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.links_filter.cLinksTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.links_filter.cLinksTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.links_filter.cLinksTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.longMessage_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'longMessage_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.longMessage_filter.cLongMessageEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.longMessage_filter.cLongMessageEnabled[0])">
                    <v-icon>{{ settings.__permission_based__.longMessage_filter.cLongMessageEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.longMessage_filter.cLongMessageTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.longMessage_filter.cLongMessageTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.longMessage_filter.cLongMessageTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.spam_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'spam_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.spam_filter.cSpamEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.spam_filter.cSpamEnabled[0])">
                    <v-icon>{{ settings.__permission_based__.spam_filter.cSpamEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.spam_filter.cSpamTriggerLength[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.spam_filter.cSpamTriggerLength[0])">
                  <v-icon>{{ settings.__permission_based__.spam_filter.cSpamTriggerLength[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.spam_filter.cSpamMaxLength[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.spam_filter.cSpamMaxLength[0])">
                  <v-icon>{{ settings.__permission_based__.spam_filter.cSpamMaxLength[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.spam_filter.cSpamTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.spam_filter.cSpamTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.spam_filter.cSpamTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
        ...
        <v-card-title>{{ translate('categories.symbols_filter') }}</v-card-title>
        <permission-tabs v-slot:default="{permissions}" :ignored="getIgnoredPermissions(settings, 'symbols_filter')">
          <v-tab-item v-for="permission of permissions" :key="permission.id" eager>
            <v-row>
              <v-col>
                <v-layout row wrap align-baseline class="ma-0">
                  ...
                  />
                  <v-btn v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'" icon @click="settings.__permission_based__.symbols_filter.cSymbolsEnabled[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.symbols_filter.cSymbolsEnabled[0])">
                    <v-icon>{{ settings.__permission_based__.symbols_filter.cSymbolsEnabled[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                  </v-btn>
                </v-layout>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.symbols_filter.cSymbolsTriggerLength[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.symbols_filter.cSymbolsTriggerLength[0])">
                  <v-icon>{{ settings.__permission_based__.symbols_filter.cSymbolsTriggerLength[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.symbols_filter.cSymbolsMaxSymbolsConsecutively[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.symbols_filter.cSymbolsMaxSymbolsConsecutively[0])">
                  <v-icon>{{ settings.__permission_based__.symbols_filter.cSymbolsMaxSymbolsConsecutively[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.symbols_filter.cSymbolsMaxSymbolsPercent[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.symbols_filter.cSymbolsMaxSymbolsPercent[0])">
                  <v-icon>{{ settings.__permission_based__.symbols_filter.cSymbolsMaxSymbolsPercent[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
            <v-row class="mt-0">
              ...
              <v-col align-self="center" cols="auto" v-if="permission.id !== '0efd7b1c-e460-4167-8e06-8aaf2c170311'">
                <v-btn icon @click="settings.__permission_based__.symbols_filter.cSymbolsTimeout[0][permission.id] = togglePermissionLock(permissions, permission.id, settings.__permission_based__.symbols_filter.cSymbolsTimeout[0])">
                  <v-icon>{{ settings.__permission_based__.symbols_filter.cSymbolsTimeout[0][permission.id] === null ? mdiLock : mdiLockOpenVariant }}</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </v-tab-item>
