  ...
  <div ref="pointer">
    <v-toolbar dense color="blue-grey darken-4">
      ...
      <v-toolbar-title class="text-button white--text">
        {{ $t('registry.plugins.listener.name') }}
      </v-toolbar-title>
    </v-toolbar>
    <v-card dark>
      <v-card-text>
        <v-autocomplete v-model="item" label="Listener" dark :items="items" style="width: 300px;" />
...
const items = computed(() => {
  return availableListeners.value.map(availableListener => ({
    text:     capitalize(translate(`registry.plugins.listener.type.${availableListener}`)),
...
onMounted(() => {
  getSocket('/core/plugins').emit('listeners', (data) => {
    ...
    availableListeners.value = listeners.sort((a, b) => {
      const A = translate(`registry.plugins.listener.type.${a}`).toLowerCase();
      const B = translate(`registry.plugins.listener.type.${b}`).toLowerCase();
