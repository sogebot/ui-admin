  ...
  <div ref="pointer">
    ...
    <v-card dark>
      <v-card-text>
        ...
        <template v-else>
        <v-switch v-model="perUser" label="Debounce per user" dense hide-details="auto" hint="Defines if user have own debounce or is global" persistent-hint />
        <v-text-field label="Wait for ms" v-model="item" hint="Number or {variableName}"/>
        </template>
      </v-card-text>
    </v-card>
  </div>
...
watch([perUser, uuid], (val) => {
  data.value = JSON.stringify({ perUser: val[0], uuid: val[1] });
});
