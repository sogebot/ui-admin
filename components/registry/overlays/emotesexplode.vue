  ...
  <v-expansion-panels v-model="model">
    ...
    <v-expansion-panel :readonly="typeof $slots.default === 'undefined'">
      ...
      <v-expansion-panel-content>
        ...
        />
        <v-text-field
          v-model.number="options.numOfEmotes"
          :label="translate('overlays.emotes.settings.cExplosionNumOfEmotes')"
          min="1"
        />
        <v-btn @click="test">
          {{ translate('overlays.emotes.settings.btnTestExplosion') }}
        </v-btn>
      </v-expansion-panel-content>
    </v-expansion-panel>
  </v-expansion-panels>
...
export default defineComponent({
  ...
  setup (props: any, ctx) {
    const model = ref(0);
    const options = ref(setDefaultOpts(props.value!.opts, 'emotesexplode'));
